...
def conv3x3(
    ...
    return nn.Conv2d(in_planes, out_planes, kernel_size=(3, 3), stride=stride, padding=1, bias=True)
...
class WideBasicBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=(3, 3), padding=1, bias=True)
        self.dropout = nn.Dropout(p=dropout_rate)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=(3, 3), stride=stride, padding=1, bias=True)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, planes, kernel_size=(1, 1), stride=stride, bias=True),
            )
