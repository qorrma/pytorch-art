...
class CIFAR10(torchvision.datasets.CIFAR10):
    def __init__(
        ...
        if self.supervised:
            self.targets, self.data = self._text_loader(
                self.targets, self.data, root=text_path)
            mint = None
            self.target_indices = []
    ...
    @staticmethod
    def _text_loader(targets, samples, root="txt"):
...
def make_labels_matrix(
    ...
    if unique_classes:
        off_value = smoothing / (num_classes*world_size)
    else:
